...
/*Structure for a tldnode
 parent, left, right - TLDNode*
...
/*Structure for the iterator over the tldlist
 size    - long
 next    - long
 TLDNode - **nodelist - pointer to a list of pointers to nodes
...
/*getHeight
 Args:
    node - TLDNode* - given node to get height of
 Returns:
    long - height of the given node
 */
/*returns the height of the given node and 0 if the node is NULL to prevent segmentation fault - restructure*/
...
/*isImbalanced
 Args:
    node - TLDNode* - node to determine if is imbalanced
 Returns:
    int  - 1 if imbalanced, 0 otherwise
 */
...
/*findImbalanced
 Args:
    node - TLDNode* - node to begin with in the search
 Returns:
    TLDNode* - pointer to the imbalanced node
 */
static TLDNode *findImbalanced(TLDNode *node){
    if(node == NULL){
        return NULL;
    }
    if(isImbalanced(node) == 1){
        return node;
    }
